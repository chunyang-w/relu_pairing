...
__global__ void compute_row_hashes_kernel(const int16_t* matrix, int64_t M, int64_t N, 
...
__global__ void compute_hashes_kernel(const int16_t* matrix, int64_t M, int64_t N, uint64_t* hashes) {
  int row = blockIdx.x * blockDim.x + threadIdx.x;
  if (row >= M) return;
...
__global__ void find_pairs_simple_kernel(const int16_t* matrix, const uint64_t* hashes, 
